...
class PointCloud(ABC):
    ...
    def from_file_multisweep(cls,
        ...
        # print(f"ref_sd_token {ref_sd_token}")
        ...
        # print(f"init sample_data_token {sample_data_token}")
        current_sd_rec = nusc.get('sample_data', sample_data_token)
        for _ in range(nsweeps):
            # print(f"current_sd_rec {current_sd_rec}")
            # Load up the pointcloud and remove points close to the sensor.
            current_pc = cls.from_file(osp.join(nusc.dataroot, current_sd_rec['filename']))
            # current_pc.remove_close(min_distance)
            # print(current_pc.points)
            # Get past pose.
            ...
            # print(f"current_pc.points {current_pc.points}")
            max_values = np.max(current_pc.points, axis=1)
            # for i, max_val in enumerate(max_values):
            # print(f"Maximum value in column {i}: {max_val}")
            ...
            # print(f"current_sd_rec {current_sd_rec}")
            # print(f"current_sd_rec['next'] {current_sd_rec['next']}")
