...
class NuScenes:
    ...
    def __init__(self,
                ...
                #  version: str = 'v1.0-trainval', #irrelevant gia export_kitti.py
    ...
    def get_sample_data(self, sample_data_token: str,
        ...
        if selected_anntokens is not None:
            ...
            # print(f"nuscenes.py boxes {boxes}")
        else:
        ...
        for box in boxes:
            if use_flat_vehicle_coordinates:
                ...
                # Move box to ego vehicle coord system. #global->ego
                # print(f"global {box.center}")
                # print(f"global {box.rotation_matrix}")
                # print(f"global {box.wlh}")
                box.translate(-np.array(pose_record['translation']))
                box.rotate(Quaternion(pose_record['rotation']).inverse)
                # print(f"ego {box.center}")
                # print(f"ego {box.rotation_matrix}")
                # print(f"ego {box.wlh}")
                #  Move box to sensor coord system. #ego->lidar
                box.translate(-np.array(cs_record['translation']))
                box.rotate(Quaternion(cs_record['rotation']).inverse)
                # print(f"lidar {box.center}")
                # print(f"lidar {box.rotation_matrix}")
                # print(f"lidar {box.wlh}")
    ...
    def get_box(self, sample_annotation_token: str) -> Box:
        ...
        # print(f"record['rotation'] {record['rotation']}")
        # print(f"Quaternion(record['rotation']) {Quaternion(record['rotation'])}")
        # print(Box(record['translation'], record['size'], Quaternion(record['rotation']),
...
class NuScenesExplorer:
    ...
    def map_pointcloud_to_image(self,
        ...
        print(f"lidar pc nuscenes: {lidar_points}")
        im = Image.open(osp.join(self.nusc.dataroot, cam['filename']))
        ...
        #print(img.size)
        print(f"lidar frame pc: {pc.points}")
        print(f"Max values in each row: {np.amax(pc.points, axis=1)}")
        # Points live in the point sensor frame. So they need to be transformed via global to the image plane.
        # First step: transform the pointcloud to the ego vehicle frame for the timestamp of the sweep.
        cs_record = self.nusc.get('calibrated_sensor', pointsensor['calibrated_sensor_token'])
        #print(cs_record)
        ...
        print(f"ego vehicle pc: {pc.points}")
        print(f"Max values in each row: {np.amax(pc.points, axis=1)}")
        # Second step: transform from ego to the global frame.
        poserecord = self.nusc.get('ego_pose', pointsensor['ego_pose_token'])
        pc.rotate(Quaternion(poserecord['rotation']).rotation_matrix)
        pc.translate(np.array(poserecord['translation']))
        print(f"global pc: {pc.points}")
        print(f"Max values in each row: {np.amax(pc.points, axis=1)}")
        # Third step: transform from global into the ego vehicle frame for the timestamp of the image.
        poserecord = self.nusc.get('ego_pose', cam['ego_pose_token'])
        pc.translate(-np.array(poserecord['translation']))
        pc.rotate(Quaternion(poserecord['rotation']).rotation_matrix.T)
        print(f"ego vehicle image timestamp pc: {pc.points}")
        print(f"Max values in each row: {np.amax(pc.points, axis=1)}")
        # Fourth step: transform from ego into the camera.
        cs_record = self.nusc.get('calibrated_sensor', cam['calibrated_sensor_token'])
        pc.translate(-np.array(cs_record['translation']))
        ...
        # print(f"points after {points.max()}")
        # Remove points that are either outside or behind the camera. Leave a margin of 1 pixel for aesthetic reasons.
        # Also make sure points are at least 1m in front of the camera to avoid seeing the lidar points on the camera
        # casing for non-keyframes which are slightly out of sync.
        # print(f"lidar_pc nuscenes {pc.points[:3, :]}")
        # print(f"img nuscenes {img}")
        # print(f"camera intrinsic nuscenes {camera_intrinsics_array}")
        # print(f"points nuscenes {points}")
        # print(f"depths nuscenes {depths}")
        mask = np.ones(depths.shape[0], dtype=bool)
        mask = np.logical_and(mask, depths > min_dist)
        mask = np.logical_and(mask, points[0, :] > 1)
        ...
        print(f"maskef_pc shape nuscenes {masked_pc.shape}") #(3, 3044)
        """
        ...
        print(f"pc.points nuscenes {pc.points.shape}")
    ...
    def render_annotation(self,
        ...
        # Display the figures
        plt.show()
        # Close the figures after viewing
        plt.close(fig)
